						...
						<div class="col-xs-12">
							<!-- PAGE CONTENT BEGINS -->
								<div class="form-group">
									<div class="col-sm-9">
										<input type="hidden" id="id_guru" name="id_guru" placeholder="ID Guru" class="form-control" value="<?php 
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Judul Seminar </label>
									<div class="col-sm-9">
										<textarea class="form-control" id="judul_seminar" name="judul_seminar" placeholder="Judul Seminar"></textarea>
									</div>
								</div>
						...
						<div class="col-xs-12">
							<!-- PAGE CONTENT BEGINS -->
								...
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Judul Seminar </label>
									<div class="col-sm-9">
										<textarea class="form-control" id="e_judul_seminar" name="e_judul_seminar" placeholder="Judul Seminar"></textarea>
									</div>
								</div>
	    ...
	    $("#formTambah").validate({
		   errorClass: "my-error-class",
		    ...
		    submitHandler: function(form) {
		      ...
		      $.ajax({
		        url: "<?php echo base_url('riwayatseminar/simpan_rwytseminar') ?>",
		        type: "POST",
...
if ($("#formEdit").length > 0) {
    $("#formEdit").validate({
	    ...
	    submitHandler: function(form) {
	      ...
	      $.ajax({
	        url: "<?php echo base_url('riwayatseminar/update_rwytseminar') ?>",
	        type: "POST",
...
<script type="text/javascript">
	...
	function show_data() {
		$.ajax({
			type: 'ajax',
			url: '<?php echo site_url('riwayatseminar/tampil_rwytseminarbyguru/'.$this->uri->segment(3)) ?>',
			async: true,
			dataType: 'json',
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('riwayatseminar/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
			...
			success: function(data) {
				$('#e_id').val(data[0].id);
				$('#e_judul_seminar').val(data[0].judul_seminar);
    ...
    $('#show_data').on('click','.item_hapus',function(){
       ...
       Swal.fire({
		  ...
		  if (result.value) {
		  	$.ajax({
				type: "POST",
				url: "<?php echo base_url('riwayatseminar/delete_rwytseminar') ?>",
				async: true,
				dataType: "JSON",
				data: {
					id: id,
